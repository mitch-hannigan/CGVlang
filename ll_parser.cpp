...
void ll_parse(const std::vector<token> &tokens, bool &result)
{
    ...
    while (token_index < tokens.size() && ll_stack.size())
    {
        ...
        {
            std::unordered_map<token_class, std::list<token_class>> &line = ll_table[ll_stack.top()];
...
std::unordered_map<token_class, std::unordered_map<token_class, std::list<token_class>>> ll_table =
    {
        // main hash_map
        {
            // first line
